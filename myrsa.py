...
#En el algoritmo se determinan 3 funciones de sustento que se complementan en f(RSA) es decir, que son fundamentales para crear y ejecutar la cuarta funcion que genera las llaves publica y privada del RSA.
...
#El algoritmo se centra basicamente en dividir el numero mayor a entre el numero menor b, mientras diferente de 0 y hasta que el resto de a/b sea 0, dado el caso de que a<b se aplica intercambio de valores a las variables, (funciona como un tipo de auxiliar que reemplaza valores).
...
#Estrategia:
#En primera instancia se confirma que p y q son numeros primos, de lo contrario se genera un tipo de break que me expulsa de la funcion.
#Si los numeros cumplen la condicion son enviados a la siguiente etapa donde se genera N equivalente a (p*q) y en su efecto se genera el algoritmo de euler.
#Para hallar e correspondiente a la llave publicaexisten dos condiciones basicas:[1 < e < phi] y coprimo con phi.
#Por ello se crea una tupla que va desde 2 hasta phi sin contar phi, se implementa el modulo random.randrange que me genera una lista de numeros aleatoriamente con el rango y argumentos dados. Para que el modulo no se base en el tiempo para generar los numeros se utiliza random.seed(1) que define una instancia desde la cual siempre va actualizarce.## El modulo requiere que se importe al file random: import random.De otro modo el programa encuentra date flotante que genera error.
#Para hallar d correspondiente a la clave privada se utiliza de igual manera el modulo random.randrange
...
# phi para hallar el numero que cumple con la condicion.
...
def rsa(p,q):
	...
	print 
		...
		print "LLave privada: ","d= ",d,',',"N= ",N #salida de llave publica
...
val=rsa(v,u)#evaluo de entradas
print val
